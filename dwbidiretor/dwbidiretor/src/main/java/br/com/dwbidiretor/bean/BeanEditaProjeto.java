...
@Named
@ViewScoped
public class BeanEditaProjeto implements Serializable {
	...
	public void carregar() {
		...
		this.projeto = (Projeto) session.getAttribute("projetoAux");	
		this.listaetapas = this.projeto.getEtapas();
		...
		session.removeAttribute("projetoAux");
	}
	...
	private void criatimelineProjeto() {
		...
		timelineProjeto.add(new TimelineEvent(projeto.getStatus(),ini,fim,false,projeto.getNome(),projeto.getStatus().toLowerCase()));
		
		for (Etapa e : listaetapas) {
			TimelineEvent event = new TimelineEvent();
			event.setData(e.getStatus());
			event.setStartDate(e.getDtinicio());
			event.setEndDate(e.getDtprevista());
			event.setGroup("Etapa: "+e.getNome());
			event.setStyleClass(e.getStatus().toLowerCase());
			
			timelineProjeto.add(event);
			
			for(Etapa_Item i:e.getItems()) {
				TimelineEvent eventi = new TimelineEvent();
				eventi.setData("Item: "+i.getDescricao()+" Status: "+i.getStatus());
				eventi.setStartDate(i.getDtinicio());
				eventi.setEndDate(i.getDtprevista());
				eventi.setGroup("Etapa: "+e.getNome());
				eventi.setStyleClass(i.getStatus().toLowerCase()+"_etapa_item");
				
				timelineProjeto.add(eventi);
			}
		}
